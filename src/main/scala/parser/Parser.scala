...
sealed trait Parser[+A] {
  ...
  def zeroOrMore[B >: A]: KleeneStarParser[B] = KleeneStarParser(this)
  def oneOrMore[B >: A]: RepeatedParser[B] = RepeatedParser(this)

  def parse(input: String): Result[A] = parse(input, 0)
  def parse(input: String, index: Int): Result[A]

  def and[B >: A: Semigroup](others: Parser[B]*): Parser[B] =
...
class KleeneStarParser[A](parser: Parser[A]) extends ValidParser[List[A]]:
  ...
  def combineWith[F[_]](empty: F[A])(combine: (F[A], A) => F[A]): Parser[F[A]] =
...
/* ------------------------------- Basic types ------------------------------ */
final case class NullParser() extends ValidParser[Null]:
...
final class LazyParser[A](parser: => Parser[A]) extends ValidParser[A]:
  lazy val cachedParser = parser
  override def parse(input: String, index: Int): Result[A] =
    cachedParser.parse(input, index)
  override def prettyPrint(level: Int): String =
    "LazyParser"
...
/* ------------------------------- Combinators ------------------------------ */
final class ParserOrElse[A, B](left: Parser[A], right: => Parser[B]) extends ValidParser[A | B]:
